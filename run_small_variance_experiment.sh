...
for i in `seq 0 $((${#config_paths[@]} - 1))`; do
    ...
    # If method is uni_mlr
if [ $method == "uni_mlr" ]; then
    python uni_mlr_trainer.py --config_path $config_path --main_path $main_path --num_epoch $num_epoch --experiment_name $experiment_name --backbone $backbone --dataset $dataset --supervision $supervision
    python uni_mlr_calculate_metrics.py  --config_path $config_path --main_path $main_path --experiment_name $experiment_name --backbone $backbone --dataset $dataset --mode $mode
    fi
