...
# UniMLR Classification Loss
def UniMLRClassification(z_mean, z_std, labels):
...
# UniMLR Ranking Loss
def UniMLRRanking(z_mean, z_logvar, labels):
...
def UniMLR(z_mean, z_logvar, labels):

    z_std = torch.exp(z_logvar / 2)
    classification_loss = UniMLRClassification(z_mean, z_std, labels)
    ranking_loss = UniMLRRanking(z_mean, z_logvar, labels)
...
def weak_UniMLR(z_mean, z_logvar, labels):
